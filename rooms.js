...
const rooms = [
    ...
    },
    {
        ...
        images: ["images/double1.jpg", "images/double2.jpg", "images/double3.jpg", "images/double4.jpg"],
        highlights: [
    ...
    },
    {
        ...
        description:
            "Perfect for two guests who prefer separate sleeping arrangements. Our Twin Room offers two comfortable single beds with luxury bedding, individual reading lights, and personal climate controls for optimal comfort. The spacious 35 m² layout provides ample room to move around, with a private balcony offering stunning city or garden views. The room features modern décor, contemporary furnishings, and a well-appointed bathroom with walk-in shower. Ideal for business colleagues, friends, or family members traveling together who appreciate personal space and comfort.",
        size: "35 M²",
        maxPeople: 2,
        price: 139,
        imageUrl: "images/twin1.jpg",
        images: ["images/twin1.jpg", "images/twin2.jpg", "images/twin3.jpg"],
        highlights: [
    ...
    },
    {
        ...
        featuredAmenities: ["King Bed", "Private Terrace", "Living Area", "Jacuzzi", "Mini Bar", "Walk-In Closet"],
        amenities: [
            "King Bed",
            "Private Terrace",
            "Living Area",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function initInjectedAdvancedModal() {
        ...
        if (advancedAmenitiesGrid) {
            ...
            advancedAmenitiesGrid.innerHTML = allAmenities
                .map(
                    (amenity) => `
                <div class="amenity-checkbox-modal">
                    <input type="checkbox" id="adv-amenity-${amenity
                        .replace(/\s+/g, "-")
                        .toLowerCase()}" value="${amenity}">
        ...
        if (advancedCancelBtn)
            advancedCancelBtn.addEventListener("click", () => advancedFilterModal.classList.remove("show"));
        // close when clicking overlay
        advancedFilterModal.addEventListener("click", (e) => {
            if (e.target === advancedFilterModal) advancedFilterModal.classList.remove("show");
        });

        // reset handler
        if (advancedResetBtn) {
            advancedResetBtn.addEventListener("click", () => {
                const min = document.getElementById("advMinPrice");
                if (min) min.value = "";
                const max = document.getElementById("advMaxPrice");
                if (max) max.value = "";
                const guests = document.getElementById("advGuests");
                if (guests) guests.value = "";
                document
                    .querySelectorAll('#advancedAmenitiesGrid input[type="checkbox"]')
        ...
        if (advancedApplyBtn) {
            advancedApplyBtn.addEventListener("click", () => {
                ...
                const selectedAdvancedAmenities = Array.from(
                    document.querySelectorAll('#advancedAmenitiesGrid input[type="checkbox"]:checked')
                ).map((cb) => cb.value);
    ...
    if (roomCardsDisplay) {
        ...
        function renderRoomCards(list, category = "all") {
            ...
            const visibleRooms = category === "all" ? list : list.filter((r) => r.category === category);
            ...
            if (category === "all") {
                categories.forEach((cat) => {
                    ...
                    const heading = `<h2 class="category-heading">| ${
                ...
                roomCardsDisplay.innerHTML = `<h2 class="category-heading">| ${
        ...
        function filterRooms() {
            ...
            const selectedCapacityRanges = Array.from(document.querySelectorAll('input[name="capacity"]:checked')).map(
                (cb) => cb.value
            );
            const selectedAmenities = Array.from(document.querySelectorAll('input[name="amenity"]:checked')).map(
                (cb) => cb.value
            );
            ...
            if (selectedAmenities.length > 0) {
                filteredList = filteredList.filter((room) => {
                    return selectedAmenities.every(
                        (amenity) =>
                            room.amenities.includes(amenity) || room.amenities.includes(amenity.split(" / ")[0])
                    );
                });
            }
